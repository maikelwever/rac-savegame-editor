...
* Altered for cross-platform compatibility by Maikel Wever (maikelwever@gmail.com) in 2019.
*
...
namespace PS3SaveManager
{
    public class Param_PFD
    {
        ...
        public bool ValidAllEntryHashes(string rootdirectory, bool fix)
        {
            for (var i = 0; i < PFDEntries.entries.Count; i++)
            {
                var filepath = Path.Combine(rootdirectory, PFDEntries.entries[i].file_name);
                if (!File.Exists(filepath))
                    return false;
        ...
        public bool RebuilParamPFD(string rootdirectory, bool encryptfiles)
        {
            try
            {
                ...
                File.WriteAllBytes(Path.Combine(rootdirectory, "/PARAM.PFD"), GetParamPFDCombinedData());
                DoProgress("Param.PFD Rebuilding complete! Rebuilded FilePath => " + Path.Combine(rootdirectory, "PARAM.PFD"),
                    MessageType.Info);
                return true;
            }
            catch (Exception ex)
        ...
        public int DecryptAllFiles(string root)
        {
            try
            {
                ...
                foreach (var t in PFDEntries.entries)
                {
                    if (t.file_name.ToLower() == "param.sfo")
                        continue;
                    var filepath = Path.Combine(root, t.file_name);
                    if (File.Exists(filepath))
